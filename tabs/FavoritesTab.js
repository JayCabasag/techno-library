...
export default function FavoritesTab({ navigation }) {
  ...
  return (
      ...
      >
        <Text style={{paddingVertical: 7}}>Books added to favorites ({favoriteBookList?.length})</Text>
        <IconButton
          icon="refresh"
          color={COLORS.RED}
          size={20}
      ...
      </View>
      {
         !isLoading && !hasFavoriteBooks && (
          <View style={{height: windowHeight/2, width: windowWidth, display: 'flex', alignItems: 'center', justifyContent: 'center'}}>
      ...
      { favoriteBookList?.map((data, index) => {
        ...
        left={props => {
          return (
            <Image {...props} source={{uri: data?.book_cover ?? IMAGES.NO_IMAGE_AVAILABLE}} style={{height: 75, width: 75, borderRadius: 15}}/>
          )
        }}
        right={props => {
          return (<View {...props} style={{display: 'flex', alignItems: 'center', justifyContent: 'center', paddingHorizontal: 10}}>
            <IconButton
              icon={'delete'}
      ...
      {isLoading && (<View style={{height: windowHeight/2, width: windowWidth, display: 'flex', alignItems: 'center', justifyContent: 'center'}}>
